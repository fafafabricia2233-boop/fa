/* =============================================================================
   ...
   1. Copie este arquivo em src/compositions/ com o nome da peca.
      Ex.: NewHairGelox.tsx
   2. Troque NewHairQualidade por NewHairGelox nas 3 ocorrencias do fim do arquivo
...
const PECA = {
  ...
  video: "newhair/qualidade_h264.mp4",
  ...
  endCard: 15.15 as number | null,  /* MEDIDO nesta fita */
...
const TITULO = {
  ...
  linhas: [
    "MÉDICO, O TRANSPLANTE",
    "TERMINA NO RESULTADO.",
    "MAS A QUALIDADE COMEÇA AQUI.",
  ],
  ...
  /* TEMPORARIO. Sai em 5.50s, que NAO e um corte medido — e a excecao vale
     explicar: os cortes desta fita caem em 1.95 (cedo demais, o titulo mal
     teria sido escrito) e 7.95 (tarde demais, espremeria o texto da dona).
     Nenhum cai perto do fim natural do gancho, entao a regra do corte nao se
     aplica aqui e forcar um deles pioraria a peca. 5.50 e o ponto que deixa
     cada trecho do texto pousar no plano que o mostra. */
  modo: "temporario" as "temporario" | "permanente",

  inicio: 0.15, // quando a primeira letra aparece
  seguraAte: 5.30, // segura ate aqui (3.8s de leitura depois de escrito)
  saiEm: 5.50, // some aqui
};
...
const CUES: Cue[] = [
  /* Texto da dona, palavra por palavra. A fita segue a mesma ordem que a frase
     dela (sala -> mesa -> enxertos -> equipe -> paciente), entao cada cue pousa
     no plano que a mostra. A 2a entra logo depois do corte medido em 7.95s. */
  {
    start: 5.6,
    end: 7.9,
    lines: [
      { text: "Começa na preparação da sala,", size: 34 },
      { text: "na organização da mesa,", size: 34 },
    ],
  },
  {
    start: 8.05,
    ...
    lines: [
      { text: "no cuidado com os enxertos,", size: 34 },
      { text: "no alinhamento da equipe", size: 34 },
    ],
  },
  {
    start: 10.5,
    end: 12.2,
    lines: [{ text: "e em cada etapa que acontece", size: 36 }],
  },
  {
    /* pousa no plano do paciente — o texto diz "paciente" e a imagem mostra */
    start: 12.35,
    end: 15.1,
    lines: [
      { text: "ANTES MESMO DO PACIENTE", size: 46, gold: true },
      { text: "PERCEBER.", size: 46, gold: true },
    ],
  },
];
...
export type NewHairQualidadeProps = {
  durationSeconds: number;
  video: string;
};

export const NewHairQualidade: React.FC<NewHairQualidadeProps> = ({ video }) => {
