...
const PECA = {
  ...
  video: "newhair/mudarumo_h264.mp4",
...
const TITULO = {
  ...
  seguraAte: 3.3, // cortes medidos aos 5.25s / 8.45s / 10.40s — longe demais pra esperar
  saiEm: 3.5, // sai antes pra nao comer o tempo das 5 legendas
};
...
const CUES: Cue[] = [
  {
    ...
    lines: [
      { text: "Nem tudo acontece exatamente", size: 30 },
      { text: "COMO FOI PLANEJADO.", size: 38, gold: true },
    ],
  },
  {
    start: 5.75,
    end: 7.1,
    lines: [{ text: "Às vezes muda a ESTRATÉGIA.", size: 38, gold: true }],
  },
  {
    start: 7.3,
    end: 8.6,
    lines: [{ text: "Muda a PRIORIDADE.", size: 42, gold: true }],
  },
  {
    start: 8.8,
    end: 10.35,
    lines: [{ text: "MUDA O RITMO.", size: 46, gold: true }],
  },
  {
    start: 10.8,
    end: 13.3,
    lines: [
      { text: "Na New Hair, a equipe precisa ter", size: 28 },
      { text: "LEITURA DE CIRURGIA", size: 40, gold: true },
      { text: "para se adaptar sem perder o fluxo.", size: 26 },
    ],
  },
];
...
export type NewHairMudaRumoProps = {
  durationSeconds: number;
  video: string;
};

export const NewHairMudaRumo: React.FC<NewHairMudaRumoProps> = ({ video }) => {
