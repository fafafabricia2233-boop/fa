...
const PECA = {
  ...
  video: "newhair/confiar_h264.mp4",
  ...
  /* MEDIDO. true = B-roll em cima e rosto embaixo (dois planos no mesmo quadro).
     O medir-fita.py acusou split nesta fita, mas e falso positivo: o quadro e um
     plano so — o topo (parede e teto brancos) e muito mais claro que a base, e o
     detector le isso como dois planos. Conferido frame a frame. */
  splitScreen: false,

  /* MEDIDO. true = fita CLARA: brilho da faixa da legenda entre 100 e 180 o tempo
     todo (as outras pecas ficam entre 35 e 90). Off-white e dourado somem em cima
     disso com o veu padrao, entao o scrim vai mais pesado. */
  fitaClara: true,
...
const TITULO = {
  linhas: [
    "MÉDICO, VOCÊ CONFIA NA",
    "SUA EQUIPE O SUFICIENTE",
    "PARA NÃO PRECISAR",
    "OLHAR PARA TRÁS?",
  ],
  ...
  seguraAte: 3.3, // plano contínuo (so dissolves), nao ha corte pra aproveitar
  saiEm: 3.5, // 7 legendas pela frente: o gancho nao pode segurar mais que isso
};
...
const CUES: Cue[] = [
  {
    ...
    lines: [
      { text: "Enquanto você está focado na técnica,", size: 28 },
      { text: "MUITA COISA CONTINUA ACONTECENDO.", size: 34, gold: true },
    ],
  },
  {
    start: 5.8,
    end: 6.85,
    lines: [{ text: "CONTAGEM.", size: 46, gold: true }],
  },
  {
    start: 6.95,
    end: 8.0,
    lines: [{ text: "ORGANIZAÇÃO.", size: 46, gold: true }],
  },
  {
    start: 8.1,
    end: 9.15,
    lines: [{ text: "CONSERVAÇÃO.", size: 46, gold: true }],
  },
  {
    start: 9.25,
    end: 10.4,
    lines: [{ text: "PRÓXIMA ETAPA.", size: 46, gold: true }],
  },
  {
    start: 10.75,
    end: 12.4,
    lines: [
      { text: "Na New Hair, o objetivo é que você saiba que", size: 26 },
      { text: "O PROCESSO CONTINUA FUNCIONANDO", size: 34, gold: true },
    ],
  },
  {
    start: 12.55,
    ...
    lines: [
      { text: "mesmo quando sua atenção está", size: 28 },
      { text: "ONDE REALMENTE PRECISA ESTAR.", size: 36, gold: true },
    ],
  },
];
...
export type NewHairConfiarProps = {
  durationSeconds: number;
  video: string;
};

export const NewHairConfiar: React.FC<NewHairConfiarProps> = ({ video }) => {
  ...
  const pesoTopo = PECA.fitaClara ? 0.86 : PECA.splitScreen ? 0.82 : 0.72;
  /* rodape tambem mais pesado na fita clara: a faixa da legenda chega a 180 de
     brilho aqui, e dourado em cima de branco some. */
  const pesoRodape = PECA.fitaClara ? 0.92 : 0.62;
  /* na fita clara o veu de baixo tambem precisa SUBIR mais: o padrao zera em 45%
     da altura e a legenda mora a ~22% do rodape, onde sobrava so 0.40 de veu —
     medido: dourado em cima de bandeja branca (brilho 180) sumia. Com a parada
     intermediaria em 25% o veu chega a ~0.58 na linha da legenda. */
  const gradRodape = PECA.fitaClara
    ? `linear-gradient(to top, rgba(11,36,54,${pesoRodape}) 0%, rgba(11,36,54,0.82) 22%, rgba(11,36,54,0.5) 34%, rgba(11,36,54,0) 62%)`
    : `linear-gradient(to top, rgba(11,36,54,${pesoRodape}) 0%, rgba(11,36,54,0) 45%)`;
              ...
              {cue.lines.map((l, j) => (
                <div
                  ...
                  style={{
                    ...
                    textAlign: "center",
                    lineHeight: 1.35,
                    /* fita clara: a sombra vira contorno. Sem isto o dourado
                       encosta em branco de bandeja (brilho 180+) e some. */
                    textShadow: PECA.fitaClara
                      ? "0 2px 10px rgba(11,36,54,0.95), 0 0 22px rgba(11,36,54,0.9)"
                      : "0 1px 16px rgba(11,36,54,0.85)",
                  }}
                >
                  {l.text}
                </div>
