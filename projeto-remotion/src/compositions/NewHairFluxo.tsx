...
const TITULO = {
  ...
  seguraAte: 3.4, // corte aos 3.70s
  saiEm: 3.6, // saída antes do corte
};
...
const CUES: Cue[] = [
  {
    ...
    lines: [{ text: "O material. A PRÓXIMA ETAPA.", size: 44, gold: true }],
  },
  {
    start: 5.4,
    end: 6.9,
    lines: [{ text: "A organização da MESA.", size: 44, gold: true }],
  },
  {
    start: 7.1,
    end: 8.9,
    lines: [{ text: "Uma equipe despreparada espera VOCÊ MANDAR", size: 44, gold: true }],
  },
  {
    start: 9.1,
    end: 10.9,
    lines: [{ text: "Na New Hair, a equipe ENTENDE O FLUXO", size: 44, gold: true }],
  },
  {
    start: 11.1,
    end: 12.7,
    lines: [{ text: "Se antecipa e trabalha para você", size: 34 }],
  },
  {
    start: 12.9,
    end: 14.9,
    lines: [
      { text: "NÃO PRECISAR SUPERVISIONAR", size: 44, gold: true },
      { text: "o básico o tempo inteiro", size: 34 },
    ],
  },
];
...
export type NewHairFluxoProps = {
  durationSeconds: number;
  video: string;
};

export const NewHairFluxo: React.FC<NewHairFluxoProps> = ({ video }) => {
