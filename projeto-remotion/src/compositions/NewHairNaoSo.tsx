/* =============================================================================
   ...
   1. Copie este arquivo em src/compositions/ com o nome da peca.
      Ex.: NewHairGelox.tsx
   2. Troque NewHairNaoSo por NewHairGelox nas 3 ocorrencias do fim do arquivo
...
const TITULO = {
  ...
  /* TEMPORARIO: sai em 2.50s, ainda dentro do plano da montagem da mesa, pra
     que a 1a palavra ("organizacao") ainda pouse nesse mesmo plano. */
...
const CUES: Cue[] = [
  /* ORDEM DA DONA: "sincronize as palavras conforme as coisas vao acontecendo
     no video". Mapeei a fita plano a plano e cada palavra pousa no trecho que
     a mostra. As viradas caem nos cortes medidos (4.70 e 9.10):

       0.0-4.7   montagem da mesa: gaze, cubas, instrumentais  -> ORGANIZAÇÃO
       4.7-6.9   enxertos dispostos um a um nas fileiras       -> CONTAGEM
       6.9-9.1   fileiras cheias e a cuba com solucao          -> CONSERVAÇÃO
       9.1-10.9  implante em andamento                         -> FLUXO
       11.0-13.2 dois membros trabalhando lado a lado          -> SINCRONIA

     Cada palavra sozinha em dourado: e uma lista de pilares, e uma por vez
     em cima da imagem que a prova. */
  {
    start: 2.65,
    end: 4.6,
    lines: [
      { text: "Existe", size: 36 },
      { text: "ORGANIZAÇÃO.", size: 48, gold: true },
    ],
  },
  {
    start: 4.8,
    end: 6.85,
    lines: [{ text: "CONTAGEM.", size: 48, gold: true }],
  },
  {
    start: 6.95,
    end: 9.0,
    lines: [{ text: "CONSERVAÇÃO.", size: 48, gold: true }],
  },
  {
    start: 9.25,
    end: 10.85,
    lines: [{ text: "FLUXO.", size: 48, gold: true }],
  },
  {
    start: 10.95,
    end: 13.15,
    lines: [{ text: "SINCRONIA.", size: 48, gold: true }],
  },
];
...
export type NewHairNaoSoProps = {
  durationSeconds: number;
  video: string;
};

export const NewHairNaoSo: React.FC<NewHairNaoSoProps> = ({ video }) => {
