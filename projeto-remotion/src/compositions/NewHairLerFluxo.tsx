...
const PECA = {
  ...
  video: "newhair/lerfluxo_h264.mp4",
...
const TITULO = {
  linhas: [
    "MÉDICO, SE VOCÊ PRECISA",
    "DIZER O PRÓXIMO PASSO",
    "O TEMPO TODO, A EQUIPE",
    "AINDA NÃO ESTÁ LENDO",
    "A SUA CIRURGIA",
  ],
  ...
  seguraAte: 3.8, // texto denso, fita curta (12.35s), corte aos 4.10s — segura menos
  saiEm: 4.0,
};
...
const CUES: Cue[] = [
  {
    ...
    lines: [{ text: "Uma etapa TERMINA.", size: 44, gold: true }],
  },
  {
    start: 5.3,
    end: 6.8,
    lines: [{ text: "A próxima já deveria estar ENCAMINHADA.", size: 34, gold: true }],
  },
  {
    start: 6.8,
    end: 8.3,
    lines: [{ text: "Na New Hair, a equipe ACOMPANHA O FLUXO", size: 32, gold: true }],
  },
  {
    start: 8.3,
    end: 9.6,
    lines: [{ text: "para ANTECIPAR NECESSIDADES", size: 36, gold: true }],
  },
  {
    start: 9.6,
    end: 10.6,
    lines: [{ text: "e REDUZIR INTERRUPÇÕES.", size: 38, gold: true }],
  },
  {
    start: 10.6,
    end: 12.0,
    lines: [{ text: "Porque suporte de verdade também é SABER O QUE VEM DEPOIS.", size: 28, gold: true }],
  },
];
...
export type NewHairLerFluxoProps = {
  durationSeconds: number;
  video: string;
};

export const NewHairLerFluxo: React.FC<NewHairLerFluxoProps> = ({ video }) => {
