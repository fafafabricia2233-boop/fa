...
const PECA = {
  ...
  video: "newhair/dependencia_h264.mp4",
...
const TITULO = {
  linhas: [
    ...
    "SE UMA PESSOA DA EQUIPE",
    "PRECISAR SAIR DA FUNÇÃO?",
  ],
  ...
  seguraAte: 4.0, // sem cortes neste video
  saiEm: 4.3, // saída suave
};
...
const CUES: Cue[] = [
  {
    ...
    lines: [{ text: "A cirurgia TRAVA?", size: 46, gold: true }],
  },
  {
    start: 5.7,
    end: 6.9,
    lines: [{ text: "Ninguém sabe ASSUMIR?", size: 46, gold: true }],
  },
  {
    start: 6.9,
    end: 8.1,
    lines: [{ text: "O fluxo SE PERDE?", size: 46, gold: true }],
  },
  {
    start: 8.3,
    end: 10.5,
    lines: [
      { text: "Na New Hair, a equipe é TREINADA", size: 42, gold: true },
      { text: "para entender o processo como um todo.", size: 32 },
    ],
  },
  {
    start: 10.7,
    ...
    lines: [
      { text: "Porque uma cirurgia não pode DEPENDER", size: 40, gold: true },
      { text: "de uma única pessoa.", size: 32 },
    ],
  },
];
...
export type NewHairDependenciaProps = {
  durationSeconds: number;
  video: string;
};

export const NewHairDependencia: React.FC<NewHairDependenciaProps> = ({ video }) => {
