...
const PECA = {
  ...
  video: "newhair/atencaopaciente_h264.mp4",
...
const TITULO = {
  linhas: [
    "MÉDICO, SUA EQUIPE",
    "PERCEBE QUANDO O",
    "PACIENTE ESTÁ DESCONFORTÁVEL",
    "OU ESPERA ELE RECLAMAR?",
  ],
  ...
  seguraAte: 4.0, // sem cortes cedo neste video — segura mais
  saiEm: 4.3,
};
...
const CUES: Cue[] = [
  {
    ...
    lines: [{ text: "Nem todo paciente VAI FALAR.", size: 38, gold: true }],
  },
  {
    start: 6.2,
    end: 7.4,
    lines: [{ text: "Às vezes ele só MUDA A EXPRESSÃO.", size: 32, gold: true }],
  },
  {
    start: 7.4,
    end: 8.3,
    lines: [{ text: "Fica mais TENSO.", size: 40, gold: true }],
  },
  {
    start: 8.3,
    end: 9.2,
    lines: [{ text: "Fica mais QUIETO.", size: 38, gold: true }],
  },
  {
    start: 9.5,
    end: 12.5,
    lines: [
      { text: "Na New Hair, ATENÇÃO ao paciente", size: 32, gold: true },
      { text: "também faz parte do PROTOCOLO.", size: 30, gold: true },
    ],
  },
  {
    start: 13.8,
    ...
    lines: [
      { text: "Porque CUIDADO não começa", size: 34, gold: true },
      { text: "só quando alguém pede.", size: 28 },
    ],
  },
];
...
export type NewHairAtencaoPacienteProps = {
  durationSeconds: number;
  video: string;
};

export const NewHairAtencaoPaciente: React.FC<NewHairAtencaoPacienteProps> = ({ video }) => {
