...
const PECA = {
  ...
  video: "newhair/reforcaconduta_h264.mp4",
...
const TITULO = {
  ...
  seguraAte: 4.0, // primeiro corte so aos 6.15s — segura mais
  saiEm: 4.3,
};
...
const CUES: Cue[] = [
  {
    ...
    lines: [
      { text: "Uma orientação sua.", size: 30 },
      { text: "Outra resposta da equipe.", size: 30, gold: true },
    ],
  },
  {
    start: 6.8,
    end: 9.0,
    lines: [{ text: "E a CONFIANÇA começa a se perder.", size: 34, gold: true }],
  },
  {
    start: 9.2,
    end: 11.0,
    lines: [
      { text: "Na New Hair, quem define", size: 30 },
      { text: "a conduta é o MÉDICO.", size: 38, gold: true },
    ],
  },
  {
    start: 11.4,
    ...
    lines: [
      { text: "A equipe orienta dentro da própria função", size: 26 },
      { text: "e sustenta aquilo que foi ALINHADO com você.", size: 26, gold: true },
    ],
  },
];
...
export type NewHairReforcaCondutaProps = {
  durationSeconds: number;
  video: string;
};

export const NewHairReforcaConduta: React.FC<NewHairReforcaCondutaProps> = ({ video }) => {
