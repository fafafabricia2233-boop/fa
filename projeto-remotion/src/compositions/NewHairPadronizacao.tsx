...
const PECA = {
  ...
  video: "newhair/padronizacao_h264.mp4",
...
const TITULO = {
  ...
  seguraAte: 4.5, // primeiro corte so aos 6.25s — segura mais
  saiEm: 4.8,
};
...
const CUES: Cue[] = [
  {
    ...
    lines: [{ text: "Um dia FUNCIONA.", size: 46, gold: true }],
  },
  {
    start: 6.5,
    end: 8.5,
    lines: [{ text: "No outro, você precisa EXPLICAR TUDO DE NOVO.", size: 36, gold: true }],
  },
  {
    start: 8.7,
    end: 10.3,
    lines: [{ text: "Na New Hair, buscamos PADRONIZAÇÃO de processo,", size: 32, gold: true }],
  },
  {
    start: 10.3,
    end: 11.8,
    lines: [{ text: "FUNÇÕES bem definidas", size: 40, gold: true }],
  },
  {
    start: 11.8,
    end: 13.0,
    lines: [{ text: "e CONTINUIDADE de trabalho.", size: 38, gold: true }],
  },
  {
    start: 13.0,
    end: 14.7,
    lines: [
      { text: "Porque o médico não deveria", size: 30 },
      { text: "RECOMEÇAR DO ZERO a cada cirurgia.", size: 34, gold: true },
    ],
  },
];
...
export type NewHairPadronizacaoProps = {
  durationSeconds: number;
  video: string;
};

export const NewHairPadronizacao: React.FC<NewHairPadronizacaoProps> = ({ video }) => {
