/* =============================================================================
   ...
   1. Copie este arquivo em src/compositions/ com o nome da peca.
      Ex.: NewHairGelox.tsx
   2. Troque NewHairResolve por NewHairGelox nas 3 ocorrencias do fim do arquivo
...
const PECA = {
  ...
  video: "newhair/resolve_h264.mp4",
  ...
  endCard: 13.50 as number | null,
  /* MEDIDO, mas com leitura. O medir-fita.py acusou 11.50 — e ali NAO ha logo
     de marca: e um corte para um plano escuro. Conferi medindo o trecho final:
     de 11.50 a 13.25 o brilho medio cai pra ~30 mas os picos seguem em 217-222,
     ou seja, ha IMAGEM (cirurgia acontecendo), nao preto. O preto de verdade so
     chega em 13.75, com o fade comecando em ~13.50.
     Obedecer ao 11.50 cru tiraria o SELO da tela enquanto ainda ha cirurgia
     aparecendo, quebrando a regra 8. Por isso 13.50: e onde o fade comeca. */
...
const TITULO = {
  /* Primeira linha do texto da dona, a unica que ela mandou em caixa alta. */
  linhas: [
    "MÉDICO, SUA EQUIPE",
    "RESOLVE?",
  ],
  ...
  /* TEMPORARIO: sai em 2.50s, no corte medido. O titulo e curto (escrito em
     0.57s), entao 2.35s na tela ja o deixam lido com folga — e sair cedo da o
     tempo que as tres frases dela precisam pra caber em ritmo de leitura. */
  modo: "temporario" as "temporario" | "permanente",

  inicio: 0.15, // quando a primeira letra aparece
  seguraAte: 2.30, // segura ate aqui
  saiEm: 2.50, // some EM CIMA do corte medido em 2.50s
};
...
const CUES: Cue[] = [
  /* As tres frases da dona, palavra por palavra. A 3a entra logo depois do
     corte medido em 9.30s e pousa no trecho escuro do fim — fundo perfeito
     pro dourado. */
  {
    start: 2.65,
    end: 6.3,
    lines: [
      { text: "Ou você ainda precisa interromper", size: 34 },
      { text: "a cirurgia para explicar o próximo passo?", size: 34 },
    ],
  },
  {
    start: 6.4,
    ...
    lines: [
      { text: "Uma equipe preparada", size: 36 },
      { text: "acompanha o procedimento.", size: 36 },
    ],
  },
  {
    start: 9.4,
    ...
    lines: [
      { text: "NÃO ESPERA ORDENS", size: 46, gold: true },
      { text: "O TEMPO INTEIRO.", size: 46, gold: true },
    ],
  },
];
...
export type NewHairResolveProps = {
  durationSeconds: number;
  video: string;
};

export const NewHairResolve: React.FC<NewHairResolveProps> = ({ video }) => {
