/* =============================================================================
   ...
   1. Copie este arquivo em src/compositions/ com o nome da peca.
      Ex.: NewHairGelox.tsx
   2. Troque NewHairCusta por NewHairGelox nas 3 ocorrencias do fim do arquivo
...
const PECA = {
  ...
  endCard: 18.35 as number | null,
  /* MEDIDO e conferido: fade de verdade no fim (43 -> 39 -> 33 -> 28 -> 23 ->
     19 -> 15 -> 11 -> 6 -> 3). */
...
const TITULO = {
  /* Texto da dona. Unica mexida: acento em "MÉDICO,". */
  linhas: [
    "MÉDICO, UMA BOA",
    "EQUIPE CUSTA.",
  ],
  ...
  /* TEMPORARIO: sai em 2.60s pra liberar o rodape — sao 8 blocos depois dele.
     Esta fita NAO tem cortes (plano continuo), entao nao ha virada pra
     aproveitar; o ponto de saida foi escolhido pelo ritmo de leitura. */
  modo: "temporario" as "temporario" | "permanente",

  inicio: 0.15, // quando a primeira letra aparece
  seguraAte: 2.40, // segura ate aqui
  saiEm: 2.60, // some aqui
};
...
const CUES: Cue[] = [
  /* Os cinco "Custa X" entram com "Custa" no corpo e o substantivo em dourado.
     A repeticao e o recurso da frase dela — assim ela fica VISIVEL na tela, e
     cada beat ganha seu acento em vez de virar cinco linhas iguais.

     Fita de 19.1s sem cortes: nao ha virada de plano pra ancorar as trocas,
     entao o ritmo foi definido pela leitura. Sobrou folga — 15 a 21 caracteres
     por segundo nas frases longas, contra os 25 da peca anterior. */
  {
    start: 2.75,
    end: 5.0,
    lines: [
  ...
  {
    start: 5.2, end: 6.4,
    lines: [{ text: "Custa", size: 36 }, { text: "TEMPO.", size: 48, gold: true }],
  },
  {
    start: 6.55, end: 7.75,
    lines: [{ text: "Custa", size: 36 }, { text: "FOCO.", size: 48, gold: true }],
  },
  {
    start: 7.9, end: 9.1,
    lines: [{ text: "Custa", size: 36 }, { text: "DESGASTE.", size: 48, gold: true }],
  },
  {
    start: 9.25, end: 10.55,
    lines: [{ text: "Custa", size: 36 }, { text: "INTERRUPÇÕES.", size: 48, gold: true }],
  },
  {
    start: 10.7, end: 12.0,
    lines: [{ text: "Custa", size: 36 }, { text: "RETRABALHO.", size: 48, gold: true }],
  },
  {
    start: 12.2,
    end: 15.2,
...
export type NewHairCustaProps = {
  durationSeconds: number;
  video: string;
};

export const NewHairCusta: React.FC<NewHairCustaProps> = ({ video }) => {
