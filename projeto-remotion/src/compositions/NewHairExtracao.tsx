/* =============================================================================
   ...
   1. Copie este arquivo em src/compositions/ com o nome da peca.
      Ex.: NewHairGelox.tsx
   2. Troque NewHairExtracao por NewHairGelox nas 3 ocorrencias do fim do arquivo
...
const PECA = {
  ...
  video: "newhair/extracao_h264.mp4",
  ...
  /* MEDIDO e conferido: sem end card. Brilho no fim fica 104-107, sem queda —
     bate com o veredito do medidor (min 54.2, max 181.8, nunca cai de vez).
     Overlay vai ate o fim da fita. */
...
const TITULO = {
  ...
  linhas: [
    "MÉDICO, O QUE ACONTECE",
    "ENTRE A EXTRAÇÃO E A",
    "IMPLANTAÇÃO?",
  ],
  ...
  /* TEMPORARIO: sai em 3.85s, pouco antes do corte medido em 3.90s — a
     virada entrega a tela quase limpa de graca (0.2s de imagem antes da 1a
     cue, que comeca em 4.05s). */
  modo: "temporario" as "temporario" | "permanente",

  inicio: 0.15, // quando a primeira letra aparece
  seguraAte: 3.65, // segura ate aqui
  saiEm: 3.85, // some aqui
};
...
const CUES: Cue[] = [
  /* "Separação, contagem, conservação e organização." veio como UMA frase so
     (virgulas e "e" de fechamento), diferente das pecas anteriores em que a
     dona listava os itens em linhas separadas com espacamento de tabela.
     Por isso ficou como UMA cue so, nao uma palavra por vez.

     Cortes medidos: 3.90, 5.05, 7.95, 9.10, 10.50, 12.05.
       4.05-6.05   mesa com instrumentais -> tira de enxertos
                   -> "Muito mais do que o paciente imagina."
       7.95-10.55  cirurgioes em P&B + lavagem dos instrumentais (cruza os
                   cortes de 9.10 e 10.50 — os 4 itens da frase cobrem
                   varias etapas do processo, nao uma imagem so)
                   -> "Separação, contagem, conservação e organização."
       10.65-13.10 fim da lavagem + resultado no couro cabeludo
                   -> "É aqui que o cuidado com cada enxerto aparece." */
  {
    start: 4.05,
    end: 6.05,
    lines: [{ text: "Muito mais do que o paciente imagina.", size: 36 }],
  },
  {
    start: 7.95,
    end: 10.55,
    lines: [
      { text: "SEPARAÇÃO, CONTAGEM,", size: 42, gold: true },
      { text: "CONSERVAÇÃO E ORGANIZAÇÃO.", size: 38, gold: true },
    ],
  },
  {
    start: 10.65,
    ...
    lines: [
      { text: "É aqui que o cuidado com", size: 34 },
      { text: "cada enxerto aparece.", size: 34 },
    ],
  },
];
...
export type NewHairExtracaoProps = {
  durationSeconds: number;
  video: string;
};

export const NewHairExtracao: React.FC<NewHairExtracaoProps> = ({ video }) => {
