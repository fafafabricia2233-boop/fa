...
const PECA = {
  ...
  video: "newhair/enxertos_h264.mp4",
...
const TITULO = {
  linhas: [
    "MÉDICO, ENQUANTO VOCÊ",
    "ESTÁ FOCADO NA CIRURGIA,",
    "QUEM ESTÁ CUIDANDO",
    "DOS SEUS ENXERTOS?",
  ],
  ...
  seguraAte: 3.3, // fita nao tem corte duro: so dissolves. Sai na dissolve de ~3.5s
  saiEm: 3.55, // saída sobre a virada de plano
};
...
const CUES: Cue[] = [
  {
    ...
    lines: [{ text: "Eles continuam FORA DO CORPO.", size: 38, gold: true }],
  },
  {
    start: 6.0,
    end: 7.5,
    lines: [{ text: "Precisam ser ORGANIZADOS.", size: 42, gold: true }],
  },
  {
    start: 7.75,
    end: 9.1,
    lines: [{ text: "CONSERVADOS.", size: 46, gold: true }],
  },
  {
    start: 9.35,
    end: 11.0,
    lines: [{ text: "MANIPULADOS COM CUIDADO.", size: 42, gold: true }],
  },
  {
    start: 11.4,
    end: 13.9,
    lines: [
      { text: "Na New Hair, preservação folicular", size: 30 },
      { text: "NÃO É UMA ETAPA ISOLADA.", size: 36, gold: true },
    ],
  },
  {
    start: 14.3,
    end: 16.3,
    lines: [{ text: "É ATENÇÃO DURANTE TODO O PROCESSO.", size: 32, gold: true }],
  },
];
...
export type NewHairEnxertosProps = {
  durationSeconds: number;
  video: string;
};

export const NewHairEnxertos: React.FC<NewHairEnxertosProps> = ({ video }) => {
