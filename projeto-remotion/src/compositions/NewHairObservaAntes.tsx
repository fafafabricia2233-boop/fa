...
const PECA = {
  ...
  video: "newhair/observaantes_h264.mp4",
...
const TITULO = {
  ...
  seguraAte: 2.8, // texto denso, fita curta (12.2s), sem cortes cedo — segura menos
  saiEm: 3.0,
};
...
const CUES: Cue[] = [
  {
    ...
    lines: [{ text: "MESA desorganizada.", size: 38, gold: true }],
  },
  {
    start: 4.0,
    end: 4.9,
    lines: [{ text: "Enxerto MAL POSICIONADO.", size: 34, gold: true }],
  },
  {
    start: 4.9,
    end: 5.6,
    lines: [{ text: "ETAPA atrasada.", size: 38, gold: true }],
  },
  {
    start: 5.6,
    end: 6.5,
    lines: [{ text: "Paciente DESCONFORTÁVEL.", size: 32, gold: true }],
  },
  {
    start: 6.8,
    end: 9.6,
    lines: [
      { text: "Na New Hair, ATENÇÃO não começa", size: 30, gold: true },
      { text: "quando alguém reclama.", size: 28 },
    ],
  },
  {
    start: 9.9,
    end: 12.0,
    lines: [{ text: "A gente OBSERVA ANTES.", size: 40, gold: true }],
  },
];
...
export type NewHairObservaAntesProps = {
  durationSeconds: number;
  video: string;
};

export const NewHairObservaAntes: React.FC<NewHairObservaAntesProps> = ({ video }) => {
