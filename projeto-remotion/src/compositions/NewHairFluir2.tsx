/* =============================================================================
   ...
   1. Copie este arquivo em src/compositions/ com o nome da peca.
      Ex.: NewHairGelox.tsx
   2. Troque NewHairFluir2 por NewHairGelox nas 3 ocorrencias do fim do arquivo
...
const PECA = {
  ...
  endCard: 10.75 as number | null,
  /* MEDIDO nesta fita — NAO herdado da versao anterior do mesmo texto, que deu
     15.40. Aqui o fim e um CORTE SECO pro preto, nao um fade: o brilho vai de
     47.6 (10.62s) pra 3.3 (10.75s) de um quadro pro outro. */
...
const TITULO = {
  /* Mesmo texto da peca NewHairFluir, fita diferente. */
  linhas: [
  ...
  /* TEMPORARIO: sai em 2.70s, no corte medido, entregando a tela pro plano
     dos cirurgioes operando — que e onde a 1a frase precisa ancorar. */
  modo: "temporario" as "temporario" | "permanente",

  inicio: 0.15, // quando a primeira letra aparece
  seguraAte: 2.50, // segura ate aqui
  saiEm: 2.70, // some EM CIMA do corte medido em 2.70s
};
...
const CUES: Cue[] = [
  /* Fita bem mais curta que a outra versao (10.84s contra 15.81s) e com so 3
     cortes: 2.70, 8.05, 10.65. Mapeamento:

       2.85-5.40  os cirurgioes operando sob o foco
                  -> "Às vezes o problema não está na técnica do médico."
                     Mesma ancora da outra versao: a fita mostra o medico
                     operando quando o texto fala da tecnica dele.
       5.60-7.90  o frasco dourado sobre a area implantada
                  -> "Está nas PEQUENAS INTERRUPÇÕES"
       8.15-10.70 o resultado, depois do corte medido em 8.05
                  -> "que acontecem durante todo o procedimento."

     A 1a cue estende um pouco alem do plano dos cirurgioes de proposito: presa
     so nele daria 28 caracteres por segundo, rapido demais pra ler. Ela ANCORA
     no plano certo, que e o que importa. */
  {
    start: 2.85,
    end: 5.4,
    lines: [
...
export type NewHairFluir2Props = {
  durationSeconds: number;
  video: string;
};

export const NewHairFluir2: React.FC<NewHairFluir2Props> = ({ video }) => {
