...
const TITULO = {
  ...
  seguraAte: 3.0, // texto muito denso, fita curta (11.13s) — segura menos
  saiEm: 3.2,
};
...
const CUES: Cue[] = [
  {
    ...
    lines: [{ text: "Cada cirurgião tem SUA TÉCNICA.", size: 34, gold: true }],
  },
  {
    start: 4.4,
    end: 5.1,
    lines: [{ text: "SEU PADRÃO.", size: 42, gold: true }],
  },
  {
    start: 5.1,
    end: 5.8,
    lines: [{ text: "SUA PREFERÊNCIA.", size: 40, gold: true }],
  },
  {
    start: 5.8,
    end: 7.3,
    lines: [
      { text: "Na New Hair, CORRIGIR", size: 36, gold: true },
      { text: "faz parte do processo.", size: 28 },
    ],
  },
  {
    start: 7.3,
    end: 8.8,
    lines: [{ text: "A equipe ESCUTA, AJUSTA e segue.", size: 30, gold: true }],
  },
  {
    start: 8.8,
    end: 11.0,
    lines: [
      { text: "Porque EGO não pode ocupar", size: 30, gold: true },
      { text: "espaço dentro da cirurgia.", size: 28 },
    ],
  },
];
...
export type NewHairEgoProps = {
  durationSeconds: number;
  video: string;
};

export const NewHairEgo: React.FC<NewHairEgoProps> = ({ video }) => {
