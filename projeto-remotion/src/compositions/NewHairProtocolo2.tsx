...
const PECA = {
  ...
  video: "newhair/protocolo2_h264.mp4",
...
const TITULO = {
  ...
  seguraAte: 4.3, // corte aos 4.55s
  saiEm: 4.55, // saída sobre o corte
};
...
const CUES: Cue[] = [
  {
    ...
    lines: [
      { text: "Erro acontece.", size: 34 },
      { text: "CORREÇÃO também precisa acontecer.", size: 42, gold: true },
    ],
  },
  {
    start: 7.8,
    end: 9.6,
    lines: [{ text: "Mas discussão, TENSÃO", size: 42, gold: true }],
  },
  {
    start: 9.6,
    end: 11.6,
    lines: [{ text: "e EXPOSIÇÃO dentro da sala", size: 42, gold: true }],
  },
  {
    start: 11.6,
    end: 13.4,
    lines: [{ text: "só PIORAM A EXPERIÊNCIA.", size: 44, gold: true }],
  },
  {
    start: 13.7,
    end: 17.3,
    lines: [
      { text: "Na New Hair, POSTURA também", size: 34 },
      { text: "faz parte do PROTOCOLO.", size: 44, gold: true },
    ],
  },
];
...
export type NewHairProtocolo2Props = {
  durationSeconds: number;
  video: string;
};

export const NewHairProtocolo2: React.FC<NewHairProtocolo2Props> = ({ video }) => {
