...
const PECA = {
  ...
  video: "newhair/cuidapaciente_h264.mp4",
  ...
  /* MEDIDO. true = o quadro e um MOSAICO (varios clipes pequenos tocando ao mesmo
     tempo, aqui 3x3). Nao e split screen: nao ha um plano em cima e outro embaixo,
     ha nove. O texto cai em cima de imagem picotada e clara, entao o veu (scrim)
     vai mais pesado — decisao da dona em 11/09, peca NewHairCuidaPaciente. */
  mosaico: true,
...
const TITULO = {
  ...
  inicio: 0.1, // quando a primeira letra aparece
  seguraAte: 2.3, // fita de 6.13s: o gancho nao pode comer o tempo das palavras
  saiEm: 2.55, // plano contínuo, nao ha corte pra aproveitar
};
...
const CUES: Cue[] = [
  {
    ...
    lines: [
      { text: "CONFORTO.", size: 42, gold: true },
      { text: "COMUNICAÇÃO.", size: 42, gold: true },
    ],
  },
  {
    start: 4.55,
    ...
    lines: [
      { text: "POSTURA.", size: 42, gold: true },
      { text: "ACOLHIMENTO.", size: 42, gold: true },
    ],
  },
];
...
export type NewHairCuidaPacienteProps = {
  durationSeconds: number;
  video: string;
};

export const NewHairCuidaPaciente: React.FC<NewHairCuidaPacienteProps> = ({ video }) => {
  ...
  const pesoTopo = PECA.mosaico ? 0.88 : PECA.splitScreen ? 0.82 : 0.72;
  /* rodape tambem mais pesado no mosaico: os quadrinhos de baixo sao claros e
     picotados, e legenda dourada em cima disso some. */
  const pesoRodape = PECA.mosaico ? 0.78 : 0.62;
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: NH.navy }}>
      ...
      <AbsoluteFill
        style={{
          opacity: scrimRodape,
          background: `linear-gradient(to top, rgba(11,36,54,${pesoRodape}) 0%, rgba(11,36,54,0) 45%)`,
        }}
      />
