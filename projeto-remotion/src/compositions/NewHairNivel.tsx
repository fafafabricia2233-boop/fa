/* =============================================================================
   ...
   1. Copie este arquivo em src/compositions/ com o nome da peca.
      Ex.: NewHairGelox.tsx
   2. Troque NewHairNivel por NewHairGelox nas 3 ocorrencias do fim do arquivo
...
const PECA = {
  ...
  endCard: 14.95 as number | null,
  /* MEDIDO e conferido: fade to black de verdade — o brilho cai de 62 (14.75s)
     pra 56, 43, 26 e 10. Numero com o significado que a regra 7 pede. */
...
const TITULO = {
  ...
  linhas: [
    "O NÍVEL DA SUA CIRURGIA",
    "TAMBÉM É PERCEBIDO AQUI",
  ],
  ...
  /* TEMPORARIO: sai em 2.85s, no corte medido, entregando a tela pro plano
     da mesa montada — que e onde a 1a palavra dela pousa. */
  modo: "temporario" as "temporario" | "permanente",

  inicio: 0.15, // quando a primeira letra aparece
  seguraAte: 2.65, // segura ate aqui
  saiEm: 2.85, // some EM CIMA do corte medido em 2.85s
};
...
const CUES: Cue[] = [
  /* SINCRONIA COM A FITA — e o que ela NAO entrega, dito na cara:
       "organizacao da mesa"  -> 3.0-5.45  a mesa montada, instrumentais
                                 alinhados no campo azul. CASA.
       "conservacao"          -> 5.65-8.35 a tira de enxertos e as cubas,
                                 emendando no gelox. CASA.
       "comunicacao"          -> 8.55-10.4  NAO ha plano de gente conversando
                                 nesta fita. Fica sobre as maos posicionando o
                                 gelox — trabalho coordenado, que ao menos nao
                                 contradiz a palavra.
       "agilidade da equipe"  -> 10.6-12.3  idem: nao ha plano de ritmo/pressa.
                                 Fica sobre o mesmo manuseio e a virada.
       "padrao ... detalhes"  -> 12.9-14.9  o paciente pronto sob o foco. CASA
                                 como fecho.
     Nao forcei correspondencia falsa nas duas do meio. */
  {
    start: 3.0,
    end: 5.45,
    lines: [{ text: "Na organização da mesa.", size: 38 }],
  },
  {
    start: 5.65,
    end: 8.35,
    lines: [{ text: "Na conservação dos enxertos.", size: 36 }],
  },
  {
    start: 8.55,
    end: 10.4,
    lines: [{ text: "Na comunicação.", size: 38 }],
  },
  {
    start: 10.6,
    end: 12.3,
    lines: [{ text: "Na agilidade da equipe.", size: 38 }],
  },
  {
    start: 12.9,
    end: 14.9,
    lines: [
      { text: "Padrão é aquilo que aparece", size: 34 },
      { text: "NOS DETALHES.", size: 48, gold: true },
    ],
  },
];
...
export type NewHairNivelProps = {
  durationSeconds: number;
  video: string;
};

export const NewHairNivel: React.FC<NewHairNivelProps> = ({ video }) => {
