...
const PECA = {
  ...
  video: "newhair/pesoresponsabilidade_h264.mp4",
...
const TITULO = {
  ...
  seguraAte: 4.0, // sem cortes neste video — segura mais
  saiEm: 4.3,
};
...
const CUES: Cue[] = [
  {
    ...
    lines: [{ text: "Não é só PINÇA.", size: 44, gold: true }],
  },
  {
    start: 5.6,
    end: 6.6,
    lines: [{ text: "Não é só CONTAGEM.", size: 40, gold: true }],
  },
  {
    start: 6.6,
    end: 7.6,
    lines: [{ text: "Não é só IMPLANTAÇÃO.", size: 38, gold: true }],
  },
  {
    start: 7.9,
    end: 11.5,
    lines: [
      { text: "São enxertos que passaram pela sua técnica", size: 26 },
      { text: "e agora dependem do CUIDADO de quem está ao redor.", size: 26, gold: true },
    ],
  },
  {
    start: 11.8,
    ...
    lines: [
      { text: "Na New Hair, cada função é", size: 32 },
      { text: "tratada com RESPONSABILIDADE.", size: 36, gold: true },
    ],
  },
];
...
export type NewHairPesoResponsabilidadeProps = {
  durationSeconds: number;
  video: string;
};

export const NewHairPesoResponsabilidade: React.FC<NewHairPesoResponsabilidadeProps> = ({ video }) => {
