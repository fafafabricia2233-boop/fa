/* =============================================================================
   ...
   1. Copie este arquivo em src/compositions/ com o nome da peca.
      Ex.: NewHairGelox.tsx
   2. Troque NewHairFlui3 por NewHairGelox nas 3 ocorrencias do fim do arquivo
...
const PECA = {
  ...
  endCard: 17.45 as number | null,
  /* MEDIDO e conferido: fade de verdade — brilho fica em 66 por 10 quadros
     seguidos e so entao cai (66->66->63->44->32->11); o MAX tambem cai junto
     (255->255->188->133->45), confirmando escurecimento real do quadro
     inteiro, nao troca de plano. */
...
const TITULO = {
  /* Texto da dona, palavra por palavra. */
  linhas: [
  ...
  /* TEMPORARIO: sai em 3.20s. So ha 3 cortes nesta fita (0.65, 9.85, 12.35) e
     o 1o e cedo demais pro titulo (mal teria sido escrito) — a saida e por
     ritmo de leitura, como em "extracao" e "suporte". */
  modo: "temporario" as "temporario" | "permanente",

  inicio: 0.15, // quando a primeira letra aparece
  seguraAte: 3.00, // segura ate aqui
  saiEm: 3.20, // some aqui
};
...
const CUES: Cue[] = [
  /* "Extração. Separação. Contagem. Implantação." veio com PONTO depois de
     cada palavra, sem virgula nem "e" de fechamento — formato de lista de
     itens isolados (como "Instrumental. Folículos. Contagem. Próximo
     passo." na peca Supervisionar), diferente da frase corrida de
     "Separação, contagem, conservação e organização." na peca Extracao.
     Por isso virou 4 cues, uma por palavra.

     Cortes medidos: 0.65, 9.85, 12.35 — so 3, e o 1o e cedo demais pra
     ancorar qualquer coisa. Mapeamento pelo que cada trecho mostra:

       0.65-9.85   mesa com instrumentais, depois a tira de enxertos sendo
                   manipulada com pinca (um plano continuo, sem corte no
                   meio — Extração e Separação dividem esse trecho por
                   tempo de leitura, nao por corte)
       9.85-12.35  bandeja cheia, fileiras organizadas -> Contagem
       12.35-17.40 cirurgioes trabalhando, depois o resultado -> Implantação,
                   e o fecho pousa no resultado */
  {
    start: 3.20,
    end: 5.60,
...
export type NewHairFlui3Props = {
  durationSeconds: number;
  video: string;
};

export const NewHairFlui3: React.FC<NewHairFlui3Props> = ({ video }) => {
