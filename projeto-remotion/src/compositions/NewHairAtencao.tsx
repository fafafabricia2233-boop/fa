/* =============================================================================
   ...
   1. Copie este arquivo em src/compositions/ com o nome da peca.
      Ex.: NewHairGelox.tsx
   2. Troque NewHairAtencao por NewHairGelox nas 3 ocorrencias do fim do arquivo
...
const PECA = {
  ...
  video: "newhair/atencao_h264.mp4",
  ...
  endCard: 14.85 as number | null,  /* MEDIDO nesta fita */
...
const TITULO = {
  ...
  linhas: [
    "MÉDICO, IMAGINE TERMINAR",
    "UMA CIRURGIA SEM PRECISAR",
  ...
  /* TEMPORARIO: sai no corte medido em 3.95s. Tem que sair cedo porque a
     regra 1 manda o rodape ficar vazio enquanto o titulo esta na tela, e o
     texto da dona precisa dos 10.7s seguintes pra caber lendo. Alem disso o
     topo do quadro fica claro (foco cirurgico) a partir de ~11s. */
  modo: "temporario" as "temporario" | "permanente",

  inicio: 0.15, // quando a primeira letra aparece
  seguraAte: 3.75, // segura ate aqui
  saiEm: 3.95, // some EM CIMA do corte medido em 3.95s
};
...
const CUES: Cue[] = [
  /* Texto da dona, palavra por palavra, quebrado em cues de no maximo duas
     linhas. A 1a comeca depois que o titulo sai (3.95s). A virada da 3a pra
     4a cai no cacho de cortes medido em 11.25-11.75. */
  {
    start: 4.1,
    end: 7.0,
    lines: [
      { text: "Quando a equipe conhece o fluxo,", size: 34 },
      { text: "entende suas funções e antecipa", size: 34 },
    ],
  },
  {
    start: 7.1,
    ...
    lines: [
      { text: "o próximo passo, você consegue", size: 34 },
      { text: "direcionar sua atenção para o que", size: 34 },
    ],
  },
  {
    start: 10.1,
    end: 11.2,
    lines: [{ text: "realmente depende de você:", size: 36 }],
  },
  {
    start: 11.35,
    end: 14.8,
    lines: [
      { text: "ESTRATÉGIA, TÉCNICA", size: 46, gold: true },
      { text: "E PACIENTE.", size: 46, gold: true },
    ],
  },
];
...
export type NewHairAtencaoProps = {
  durationSeconds: number;
  video: string;
};

export const NewHairAtencao: React.FC<NewHairAtencaoProps> = ({ video }) => {
