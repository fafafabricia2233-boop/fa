...
const PECA = {
  ...
  video: "newhair/ansioso_h264.mp4",
...
const TITULO = {
  ...
  seguraAte: 3.0, // texto denso, fita curta — segura menos
  saiEm: 3.2,
};
...
const CUES: Cue[] = [
  {
    ...
    lines: [{ text: "O tom de voz. A postura.", size: 34 }],
  },
  {
    start: 4.5,
    end: 5.5,
    lines: [{ text: "O clima da sala.", size: 40, gold: true }],
  },
  {
    start: 5.5,
    end: 6.3,
    lines: [{ text: "A forma como ele é TRATADO.", size: 38, gold: true }],
  },
  {
    start: 6.5,
    end: 8.3,
    lines: [
      { text: "Na New Hair, ACOLHIMENTO", size: 40, gold: true },
      { text: "também faz parte do PROTOCOLO.", size: 34, gold: true },
    ],
  },
  {
    start: 8.5,
    ...
    lines: [
      { text: "Porque SEGURANÇA não é só técnica.", size: 38, gold: true },
      { text: "É também o que a equipe TRANSMITE.", size: 34, gold: true },
    ],
  },
];
...
export type NewHairAnsiosoProps = {
  durationSeconds: number;
  video: string;
};

export const NewHairAnsioso: React.FC<NewHairAnsiosoProps> = ({ video }) => {
