/* =============================================================================
   ...
   1. Copie este arquivo em src/compositions/ com o nome da peca.
      Ex.: NewHairGelox.tsx
   2. Troque NewHairOrganizada por NewHairGelox nas 3 ocorrencias do fim do arquivo
...
const PECA = {
  ...
  video: "newhair/organizada_h264.mp4",
  ...
  endCard: 7.25 as number | null,
  /* MEDIDO e conferido: desta vez o numero tem o significado que a regra 7
     pede. Medi o trecho final — o brilho cai de 41 (7.12s) pra 31, 14 e 5,
     ou seja um fade to black de verdade comecando em 7.25. */
...
const TITULO = {
  ...
  /* TEMPORARIO: sai em 2.35s. Fita curta (7.65s) e texto denso, entao o
     titulo tem que liberar o rodape cedo. */
  modo: "temporario" as "temporario" | "permanente",

  inicio: 0.15, // quando a primeira letra aparece
  seguraAte: 2.15, // segura ate aqui
  saiEm: 2.35, // some aqui
};
...
const CUES: Cue[] = [
  /* Cada frase pousa no plano que a mostra, e as viradas caem nos cortes
     medidos (3.70 e o cacho 4.75-5.10):

       ate 3.7    a mesa montada, cubas e instrumentais alinhados
                  -> "Não é sobre uma mesa bonita." A fita mostra a mesa
                     impecavel exatamente quando o texto diz que nao e sobre
                     isso. A ironia e da propria fita.
       3.7-5.1    a tira de enxertos e o instrumental a mao
                  -> "É sobre encontrar tudo rapidamente,"
       5.1-7.2    a equipe trabalhando no paciente
                  -> "reduzir interrupções e manter O PROCEDIMENTO FLUINDO."
  */
  {
    start: 2.45,
    end: 3.65,
    lines: [{ text: "Não é sobre uma mesa bonita.", size: 36 }],
  },
  {
    start: 3.8,
    end: 5.2,
    lines: [{ text: "É sobre encontrar tudo rapidamente,", size: 34 }],
  },
  {
    start: 5.35,
    end: 7.2,
    lines: [
      { text: "reduzir interrupções e manter", size: 34 },
      { text: "O PROCEDIMENTO FLUINDO.", size: 46, gold: true },
    ],
  },
];
...
export type NewHairOrganizadaProps = {
  durationSeconds: number;
  video: string;
};

export const NewHairOrganizada: React.FC<NewHairOrganizadaProps> = ({ video }) => {
