...
const PECA = {
  ...
  video: "newhair/antecipar_h264.mp4",
...
const TITULO = {
  ...
  seguraAte: 2.75, // corte aos 3.00s
  saiEm: 3.0, // saída sobre o corte
};
...
const CUES: Cue[] = [
  {
    ...
    lines: [{ text: "Porque cumprir ordem É O MÍNIMO.", size: 40, gold: true }],
  },
  {
    start: 6.0,
    end: 7.2,
    lines: [{ text: "Uma boa equipe OBSERVA.", size: 42, gold: true }],
  },
  {
    start: 7.2,
    end: 8.3,
    lines: [{ text: "ENTENDE O FLUXO.", size: 44, gold: true }],
  },
  {
    start: 8.7,
    end: 9.9,
    lines: [{ text: "SE ANTECIPA.", size: 46, gold: true }],
  },
  {
    start: 10.1,
    end: 13.9,
    lines: [
      { text: "Na New Hair, a equipe é TREINADA", size: 38, gold: true },
      { text: "para perceber o que precisa ser feito", size: 32 },
    ],
  },
  {
    start: 14.2,
    end: 16.9,
    lines: [{ text: "ANTES DE VOCÊ PRECISAR PEDIR.", size: 40, gold: true }],
  },
];
...
export type NewHairAnteciparProps = {
  durationSeconds: number;
  video: string;
};

export const NewHairAntecipar: React.FC<NewHairAnteciparProps> = ({ video }) => {
