/* =============================================================================
   ...
   1. Copie este arquivo em src/compositions/ com o nome da peca.
      Ex.: NewHairGelox.tsx
   2. Troque NewHairFlui4 por NewHairGelox nas 3 ocorrencias do fim do arquivo
...
const PECA = {
  ...
  endCard: 15.10 as number | null,
  /* MEDIDO nesta fita — NAO herdado da versao anterior do mesmo texto, que
     tinha 17.45. Conferido como fade real: brilho oscila normal ate os
     ultimos quadros e so entao cai (102->82->60->34->7), com o MAX caindo
     junto (255->218->163->92->23) — escurecimento do quadro inteiro. */
...
const TITULO = {
  /* Mesmo texto da peca NewHairFlui3, fita diferente. */
  linhas: [
    "UMA CIRURGIA QUE FLUI",
    "NÃO ACONTECE",
    "POR ACASO.",
  ],
  ...
  /* TEMPORARIO: sai em 2.85s, NAO em cima de corte (o 1o corte real e em
     3.45s, tarde demais — titulo mal teria sido lido). Ritmo de leitura. */
  modo: "temporario" as "temporario" | "permanente",

  inicio: 0.15, // quando a primeira letra aparece
  seguraAte: 2.65, // segura ate aqui
  saiEm: 2.85, // some aqui
};
...
const CUES: Cue[] = [
  /* MESMO TEXTO da NewHairFlui3, FITA DIFERENTE — cortes medidos nesta fita
     divergem em tudo da anterior (3.45, 6.55, 8.35, 10.45, 12.95 contra
     0.65, 9.85, 12.35), entao o mapeamento foi refeito do zero, nao herdado.

     ORDEM DAS 4 PALAVRAS MANTIDA (Extração, Separação, Contagem,
     Implantação) igual ao texto dela — no que muda e ONDE cada uma pousa:

       3.45-6.55   mesa com instrumentais -> tira de enxertos com pinca
                   -> Extração. e Separação. (Separação literal: e o
                   quadro que mostra a separacao dos enxertos)
       6.55-10.45  cirurgioes trabalhando (dois cortes seguidos, mesma
                   cena) -> Contagem. (sem correspondencia literal, e
                   sequencial) e Implantação. (literal: e a implantacao
                   acontecendo)
       10.45-15.05 lavagem dos instrumentais + resultado
                   -> "Organização também faz parte da técnica." (a
                   lavagem/organizacao dos instrumentais casa bem com
                   "organização", e o fecho pousa no resultado) */
  {
    start: 2.85,
    end: 5.00,
    lines: [{ text: "Cada etapa precisa conversar", size: 34 },
            { text: "com a próxima.", size: 34 }],
  },
  {
    /* Duracao minima ~0.53s (16 frames) por causa do fadeRise do template
       (8 quadros de entrada + 8 de saida); abaixo disso o interpolate quebra
       com "inputRange must be strictly monotonically increasing". Ja bateu
       nisso aqui — cue original de 0.40s (12 frames) derrubou o still. */
    start: 5.15,
    end: 5.75,
    lines: [{ text: "Extração.", size: 46, gold: true }],
  },
  {
    start: 5.85,
    end: 6.45,
    lines: [{ text: "Separação.", size: 46, gold: true }],
  },
  {
    start: 6.65,
    end: 7.65,
  ...
  },
  {
    ...
    lines: [{ text: "Implantação.", size: 46, gold: true }],
  },
  {
    start: 10.60,
    end: 14.95,
    lines: [
      { text: "Organização também", size: 34 },
      { text: "faz parte da técnica.", size: 34 },
    ],
  },
];
...
export type NewHairFlui4Props = {
  durationSeconds: number;
  video: string;
};

export const NewHairFlui4: React.FC<NewHairFlui4Props> = ({ video }) => {
