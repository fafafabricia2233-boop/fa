...
const PECA = {
  ...
  video: "newhair/leitura_h264.mp4",
...
const TITULO = {
  ...
  seguraAte: 3.25, // corte aos 3.50s
  saiEm: 3.5, // saída sobre o corte
};
...
const CUES: Cue[] = [
  {
    ...
    lines: [{ text: "Nem todo caso RESPONDE IGUAL.", size: 38, gold: true }],
  },
  {
    start: 5.4,
    end: 6.3,
    lines: [{ text: "Nem todo enxerto SE COMPORTA IGUAL.", size: 32, gold: true }],
  },
  {
    start: 6.6,
    end: 8.2,
    lines: [{ text: "Nem toda etapa exige A MESMA CONDUTA.", size: 30, gold: true }],
  },
  {
    start: 8.7,
    end: 11.9,
    lines: [
      { text: "Na New Hair, a equipe OBSERVA, COMUNICA", size: 28, gold: true },
      { text: "e se adapta ao que a cirurgia está pedindo.", size: 26 },
    ],
  },
  {
    start: 12.4,
    ...
    lines: [
      { text: "Porque trabalhar no AUTOMÁTICO", size: 32, gold: true },
      { text: "não combina com um procedimento de alto nível.", size: 24 },
    ],
  },
];
...
export type NewHairLeituraProps = {
  durationSeconds: number;
  video: string;
};

export const NewHairLeitura: React.FC<NewHairLeituraProps> = ({ video }) => {
