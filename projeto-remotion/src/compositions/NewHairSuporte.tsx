/* =============================================================================
   ...
   1. Copie este arquivo em src/compositions/ com o nome da peca.
      Ex.: NewHairGelox.tsx
   2. Troque NewHairSuporte por NewHairGelox nas 3 ocorrencias do fim do arquivo
...
const PECA = {
  ...
  video: "newhair/suporte_h264.mp4",
  ...
  /* MEDIDO e conferido: sem end card. Brilho no fim fica em 97-101, sem
     queda — bate com o veredito do medidor (min 41.4 e max 160.3, nunca cai
     de vez). Overlay vai ate o fim da fita. */
...
const TITULO = {
  ...
  /* TEMPORARIO: sai em 3.35s. Fita SEM cortes (plano continuo), entao nao ha
     virada pra ancorar a saida — e ritmo de leitura. Escolhido pra sair
     pouco antes do plano dos cirurgioes operando (~4s), que e onde a 1a cue
     ("Você contrata para ter suporte.") pousa — a imagem da equipe inteira
     em campo casa com a palavra "suporte". */
  modo: "temporario" as "temporario" | "permanente",

  inicio: 0.15, // quando a primeira letra aparece
  seguraAte: 3.15, // segura ate aqui
  saiEm: 3.35, // some aqui
};
...
const CUES: Cue[] = [
  /* Fita sem cortes: as 3 frases da dona nao tem virada de plano pra ancorar,
     entao o ritmo e so de leitura. Mesmo assim ficaram bem posicionadas pelo
     que a fita mostra em cada trecho:

       3.55-5.55   cirurgioes operando (equipe completa em campo)
                   -> "Você contrata para ter suporte."
       5.70-8.30   fim dos cirurgioes + lavagem dos instrumentais
                   -> "Para que cada etapa aconteça no momento certo."
       8.45-13.10  lavagem + resultado no couro cabeludo
                   -> "E você possa focar no que só você pode fazer." */
  {
    start: 3.55,
    end: 5.55,
    lines: [{ text: "Você contrata para ter suporte.", size: 36 }],
  },
  {
    start: 5.70,
    end: 8.30,
    lines: [
      { text: "Para que cada etapa aconteça", size: 34 },
      { text: "NO MOMENTO CERTO.", size: 46, gold: true },
    ],
  },
  {
    start: 8.45,
    ...
    lines: [
      { text: "E você possa focar no que", size: 34 },
      { text: "SÓ VOCÊ PODE FAZER.", size: 46, gold: true },
    ],
  },
];
...
export type NewHairSuporteProps = {
  durationSeconds: number;
  video: string;
};

export const NewHairSuporte: React.FC<NewHairSuporteProps> = ({ video }) => {
