/* =============================================================================
   ...
   1. Copie este arquivo em src/compositions/ com o nome da peca.
      Ex.: NewHairGelox.tsx
   2. Troque NewHairMuitasMaos por NewHairGelox nas 3 ocorrencias do fim do arquivo
...
const PECA = {
  ...
  video: "newhair/muitasmaos_h264.mp4",
  ...
  endCard: 14.60 as number | null,
  /* MEDIDO e conferido: fade de verdade — 105 -> 103 -> 97 -> 77 -> 54 -> 25. */
...
const TITULO = {
  /* Texto da dona. Unica mexida: "MEDICO" -> "MÉDICO," com acento, como nas
     outras pecas da frente. Ortografia, nao troca de palavra. */
  linhas: [
    "MÉDICO, SUA TÉCNICA",
    "PODE SER EXCELENTE.",
  ],
  ...
  /* TEMPORARIO: sai em 2.45s. Sao 10 blocos de texto depois dele numa fita de
     14.8s — o titulo tem que liberar o rodape cedo pra tudo caber lendo. */
  modo: "temporario" as "temporario" | "permanente",

  inicio: 0.15, // quando a primeira letra aparece
  seguraAte: 2.25, // segura ate aqui
  saiEm: 2.45, // some aqui
};
...
const CUES: Cue[] = [
  /* Cortes medidos: 0.95, 1.65, 3.15, 4.60, 6.80, 8.75, 11.15, 12.65.
     Nove planos: cirurgioes operando · mesa montada · tira de enxertos ·
     bandejas cheias · sala e paciente · maos em P&B · frasco dourado · resultado.

     SINCRONIA PARCIAL, dito na cara: a ordem das sete palavras e dela e nao
     bate com a ordem dos planos. Casam de verdade: ORGANIZAÇÃO e CONTAGEM
     (bandejas de enxertos alinhados), COMUNICAÇÃO (as duas maos em P&B
     trabalhando juntas) e BIOSSEGURANÇA (a sala montada). AGILIDADE,
     CONSERVAÇÃO e IMPLANTAÇÃO caem sobre planos vizinhos que nao as mostram
     literalmente. Nao reordenei o texto dela pra forcar casamento.

     A ultima frase entra depois do corte de 12.65s, pousando no plano do
     resultado — a imagem fecha o argumento junto com o texto. Antes dela ha
     0.2s so de imagem, uma respirada antes do remate. */
  {
    start: 2.45,
    end: 4.55,
    lines: [
      { text: "MAS O QUE ACONTECE AO REDOR", size: 34 },
      { text: "DELA TAMBÉM IMPORTA.", size: 34 },
    ],
  },
  { start: 4.7,  end: 5.4,   lines: [{ text: "ORGANIZAÇÃO.", size: 48, gold: true }] },
  { start: 5.55, end: 6.25,  lines: [{ text: "AGILIDADE.", size: 48, gold: true }] },
  { start: 6.4,  end: 7.1,   lines: [{ text: "CONTAGEM.", size: 48, gold: true }] },
  { start: 7.25, end: 8.15,  lines: [{ text: "CONSERVAÇÃO FOLICULAR.", size: 44, gold: true }] },
  { start: 8.3,  end: 9.0,   lines: [{ text: "IMPLANTAÇÃO.", size: 48, gold: true }] },
  { start: 9.15, end: 9.85,  lines: [{ text: "COMUNICAÇÃO.", size: 48, gold: true }] },
  { start: 10.0, end: 10.75, lines: [{ text: "BIOSSEGURANÇA.", size: 48, gold: true }] },
  {
    start: 10.9,
    end: 12.55,
    lines: [{ text: "Cada detalhe faz parte do processo.", size: 36 }],
  },
  {
    start: 12.75,
    end: 14.55,
    lines: [
      { text: "Uma boa cirurgia é construída", size: 36 },
      { text: "por muitas mãos.", size: 36 },
    ],
  },
];
...
export type NewHairMuitasMaosProps = {
  durationSeconds: number;
  video: string;
};

export const NewHairMuitasMaos: React.FC<NewHairMuitasMaosProps> = ({ video }) => {
